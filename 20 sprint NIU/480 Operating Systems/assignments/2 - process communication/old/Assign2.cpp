/***********************************************************
  ...
  Purpose:		This assignment involves using the LINUX system function pipe() as well as fork().
				...
				Their communication will run in a ring like this:
					  ...
					  --------------->------------->-------------->------------
										  Pipe C
my thinking -- 	so, Grandchild reads from C and writes to B so it should be DoWork(C[0],B[1]);
				so,    parent reads from B and writes to C so it should be DoWork(B[0],C[1]);
				so,     child reads from B and writes to A so it should be DoWork(B[0],A[1]);
