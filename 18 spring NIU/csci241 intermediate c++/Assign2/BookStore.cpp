...
*/
int BookStore::searchForISBN(const char* searchISBN)  const//char char * searchISBN [] then it refers to data in the array
...
void BookStore::processOrders(const string& fileName)
   ...
   while (inFile >> orderNumber)
      ...
      if (index == -1)
         ...
         cout << "Order #" << orderNumber << ": ISBN " << isbn << ", " << numShipped << " of "
              << orderQuantity << " shipped, order total $" 
              << (numShipped * bookArray[index].getPrice()) << endl;
         }
      }
